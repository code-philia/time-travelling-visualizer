...
import { getBackGroundMesh,  updateStateForInstance , getTestingData, getLabeledData, getCustomSelection, getUnLabelData} from './globalState';
declare global {
  interface Window {
    selectedList: any,
  }
...
export class scatterPlotVisualizerTriangles implements ScatterPlotVisualizer {
  ...
  addbackgroundImg(imgUrl: string) {
    //移除上一个画布
    if (getBackGroundMesh(this.instanceId)) {
      this.scene.remove(getBackGroundMesh(this.instanceId))
    }
    if (!imgUrl) {
      return
    }
    ...
    img.onload = () => {
      ...
      updateStateForInstance(this.instanceId, {backgroundMesh:new THREE.Mesh(plane_geometry, material)})
      // getBackgroundMesh(this.instanceId) = new THREE.Mesh(plane_geometry, material);
      this.scene.add(getBackGroundMesh(this.instanceId));
    }
  }
  createTriangles() {
    ...
    if (!getCustomSelection(this.instanceId)) {
      ...
      // this.state.customSelection.sort(func)
      // Create a new sorted array
      const sortedSelection = getCustomSelection(this.instanceId).sort(func);

// Update the state with the new sorted array
      updateStateForInstance(this.instanceId, { customSelection: sortedSelection });

      console.log('cuscus', getCustomSelection(this.instanceId));
    }
    ...
    for (let i = 0; i < pointCount * 2; i++) {
      ...
      let top = 40 / scale;
      if (getUnLabelData(this.instanceId).indexOf(Math.floor(i / 2)) !== -1) {
        ...
        positionObject.setXY(lettersSoFar * VERTICES_PER_GLYPH + 1, -left, left);
        positionObject.setXY(lettersSoFar * VERTICES_PER_GLYPH + 2, 0, top / 4);
      } else if (getTestingData(this.instanceId).indexOf(Math.floor(i / 2)) !== -1) {
        //juxing
