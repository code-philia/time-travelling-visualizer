...
"""
Api: get all epoch data in a single request (optimized batch API)
...
Response:
    ...
    projection_neighbors (list)
    prediction (list, optional)
...
def get_epoch_data():
    ...
    vis_id = req['vis_id']
    epoch = int(req['epoch'])
    task_type = req.get('task_type', 'Classification')
    include_background = req.get('include_background', True)
    
    try:
        # Load all data for this epoch in one request
        projection, scope = load_projection(content_path, vis_id, epoch)
        original_neighbors = calculate_high_dimensional_neighbors(content_path, epoch)
        ...
        # Add prediction data for classification tasks
        if task_type == 'Classification':
            prediction = load_single_attribute(content_path, epoch, 'prediction')
            result_data['prediction'] = prediction
            
            # Optionally include background
            if include_background:
                try:
                    result_data['background'] = load_background(content_path, vis_id, epoch)
                except:
                    result_data['background'] = ''
        
        return make_response(jsonify(result_data), 200)
    except Exception as e:
        print(f"Error in get_epoch_data: {e}")
        return make_response(jsonify({'error_message': str(e)}), 400)


"""
Api: get multiple epochs data in a single request (batch API for maximum efficiency)
...
Request:
    ...
    task_type (str): 'Classification' or 'Regression'
Response:
    epochs_data (dict): { epoch_num: { projection, scope, original_neighbors, projection_neighbors, prediction?, background? } }
"""
...
def get_batch_epoch_data():
    ...
    content_path = req['content_path']
    vis_id = req['vis_id']
    epochs = req['epochs']  # List of epoch numbers
    task_type = req.get('task_type', 'Classification')
    include_background = req.get('include_background', False)  # Default false for batch to save bandwidth
    ...
    try:
        ...
        for epoch in epochs:
            ...
            if task_type == 'Classification':
                prediction = load_single_attribute(content_path, epoch, 'prediction')
                epoch_data['prediction'] = prediction
                
                if include_background:
                    try:
                        epoch_data['background'] = load_background(content_path, vis_id, epoch)
                    except:
                        epoch_data['background'] = ''
            
            epochs_data[epoch] = epoch_data
        
        return make_response(jsonify({'epochs_data': epochs_data}), 200)
    except Exception as e:
        print(f"Error in get_batch_epoch_data: {e}")
        return make_response(jsonify({'error_message': str(e)}), 400)
