...
class TrustTrainer(SingleVisTrainer):
    ...
    def train_step(self,data_provider,iteration):
        ...
        print('umap:{:.4f}\trecon_l:{:.4f}\tb_loss{:.4f}\ttemporal_l:{:.4f}\tloss:{:.4f}'.format(sum(umap_losses) / len(umap_losses),
                                                                sum(recon_losses) / len(recon_losses),
                                                                b_loss, sum(temporal_losses) / len(temporal_losses), sum(all_loss) / len(all_loss)))
        return self.loss
