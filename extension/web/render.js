...
function drawCanvas(res) {
    ...
    // const init_x = (window.vueApp.sceneBoundary.x_max + window.vueApp.sceneBoundary.x_min) / 2;
    // const init_y = (window.vueApp.sceneBoundary.y_max + window.vueApp.sceneBoundary.y_min) / 2;
    // window.vueApp.camera.position.set(init_x, init_y, 100); // This will set 3d position of camera, it is important to ensure camera's angle of view will not distort 
    ...
    img.onload = () => {
        ...
        newMesh = new THREE.Mesh(plane_geometry, material);
    ...
    container.addEventListener('mousemove', function (e) {
        if (isDragging) {
            ...
            // minX = cameraBounds.minX + (window.vueApp.camera.right - window.vueApp.camera.left) / 2 / currentZoom;
            // maxX = cameraBounds.maxX - (window.vueApp.camera.right - window.vueApp.camera.left) / 2 / currentZoom;
            // minY = cameraBounds.minY + (window.vueApp.camera.top - window.vueApp.camera.bottom) / 2 / currentZoom;
            // maxY = cameraBounds.maxY - (window.vueApp.camera.top - window.vueApp.camera.bottom) / 2 / currentZoom;
    ...
    container.addEventListener('mouseup', function (e) {
        isDragging = false;
        container.style.cursor = 'default';
        // console.log(`Camera: (${window.vueApp.camera.position.x}, ${window.vueApp.camera.position.y}), ${window.vueApp.camera.zoom}`);
        // console.log(cameraBounds);
        // console.log(window.vueApp.camera);
        // console.log(`${newMesh.width}, ${newMesh.position.x}, ${newMesh.position.y}`);
    });
    ...
    var ambientLight = new THREE.AmbientLight(0xffffff, 1); // The second parameter is the light intensity
