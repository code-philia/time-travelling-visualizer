...
export function BottomPanel({ defaultActiveTab = '1' }: BottomPanelProps) {
    ...
    // assemble tokens
    const { tokenList, labels } = useDefaultStore(['tokenList', 'labels']);
    const groupedTokens: Record<string, string[]> = {};
    defaultGroupNames.forEach((groupName, i) => {
        groupedTokens[groupName] = [];
        labels.forEach((groupId, j) => {
            if (groupId === i) {
                groupedTokens[groupName].push(tokenList[j]);
            }
        });
    });

    const { originalText } = useDefaultStore(['originalText']);
    const { hoveredIndex, selectedIndices, setHoveredIndex, setSelectedIndices } = useDefaultStore(['hoveredIndex', 'selectedIndices', 'setHoveredIndex', 'setSelectedIndices']);
    ...
    }, [hoveredIndex, setHighlightedIndex]);
    
    useEffect(() => {
        setLockedIndices(selectedIndices);
    }, [selectedIndices, setLockedIndices]);
    ...
    let showLossAttribution = true;
    let showTokensWeightAsSize = true;
    let showTokensAlignmentAsColor = true;
            ...
            <div className='tokens-overview-container'>
                {
                    textGroups.map((key, i) => {
                        ...
                        const onChangeHighlightIndex = (index: number | null) => {
                            const remappedIndex = index === null ? undefined : remap.from(index)?? undefined;
                            setHoveredIndex(remappedIndex);
                            notifyHoveredIndexSwitch(remappedIndex);
                        };
                        ...
                        const onChangeLockedIndices = (indices: number[]) => {
                            ...
                            setSelectedIndices(remappedIndices);
                            notifySelectedIndicesSwitch(remappedIndices);
                        };
