...
export const ChartComponent = memo(() => {
    ...
    const { epoch, allEpochData, globalBounds } = useDefaultStore([
        "epoch",
        "allEpochData",
        "globalBounds",
    ]);
    const { inherentLabelData, colorDict, labelDict, textData } =
        useDefaultStore([
            "inherentLabelData",
            "colorDict",
            "labelDict",
    ...
    const { shownData, index, isFocusMode, focusIndices } = useDefaultStore([
        "shownData",
        "index",
        "isFocusMode",
        "focusIndices",
    ...
    const { revealOriginalNeighbors, revealProjectionNeighbors } =
        useDefaultStore([
            "revealOriginalNeighbors",
            "revealProjectionNeighbors",
        ]);
    const { showLabel, showIndex } = useDefaultStore([
        "showLabel",
        "showIndex",
    ]);
    ...
    let [viewportState, setViewportState] = useState<ViewportState | null>(
        null,
    );
    ...
    const prepared = useMemo<PreparedEmbedding | null>(() => {
        ...
        return {
            ...
            dataPoints,
            categoryColors:
                categoryColorList.length > 0 ? categoryColorList : null,
        };
    ...
    useEffect(() => {
        setTrailRefresh((v) => v + 1);
    }, [selectedIndices]);

    const neighborOverlayProps = useMemo(() => {
        const neighborsLoaded = epochData?.neighborsLoaded;
        if (!neighborsLoaded) {
        }

        if (!prepared || !epochData || !epochData.projection)
            return {
                center: null,
                original: [],
                projection: [],
                dataX: new Float32Array(0),
                dataY: new Float32Array(0),
                pointSize,
                revealOriginalNeighbors,
                revealProjectionNeighbors,
            } as any;
        ...
        const orig = (epochData?.originalNeighbors?.[hoverId] ?? []).filter(
            (nid) => posMap.has(nid),
        );

        const proj = (epochData?.projectionNeighbors?.[hoverId] ?? []).filter(
            (nid) => posMap.has(nid),
        );
    ...
    class NeighborOverlay {
        ...
        render() {
            ...
            const centerLoc = center
                ? this.proxy.location(center.x, center.y)
                : null;
            const neighborGroup = document.createElementNS(
                "http://www.w3.org/2000/svg",
                "g",
            );
            const trailGroup = document.createElementNS(
                "http://www.w3.org/2000/svg",
                "g",
            );
            ...
            if (centerLoc) {
                if (revealOriginalNeighbors) {
                    original.forEach((nid: number) =>
                        drawNeighbor(nid, COLOR_ORIG),
                    );
                }
                if (revealProjectionNeighbors) {
                    projection.forEach((nid: number) =>
                        drawNeighbor(nid, COLOR_PROJ),
                    );
                }
            ...
            if (this.props.showLabel || this.props.showIndex) {
                ...
                for (let i = 0; i < this.props.dataX.length; i++) {
                    ...
                    const loc = this.proxy.location(x, y);
                    const labelTextData =
                        this.props.textData && this.props.textData[id]
                            ? this.props.textData[id]
                            : (this.props.labelDict?.get(
                                  this.props.inherentLabelData[id],
                              ) ?? "");
                    let content = "";
                    if (this.props.showLabel && this.props.showIndex) {
                    ...
                    if (
                        boxX < 0 ||
                        boxY < 0 ||
                        boxX + boxWidth > this.props.proxy.width ||
                        boxY + boxHeight > this.props.proxy.height
                    ) {
        ...
        update(nextProps: Partial<any>) {
            ...
            if (this.svg) {
                ...
                this.svg.setAttribute(
                    "height",
                    String(this.props.proxy.height),
                );
            }
            this.render();
        }
    ...
    async function querySelection(
        ...
        for (let i = 0; i < simpleData.x.length; i++) {
            let d2 =
                (simpleData.x[i] - x) * (simpleData.x[i] - x) +
                (simpleData.y[i] - y) * (simpleData.y[i] - y);
        ...
        if (
            minIndex == null ||
            minDistance2 == null ||
            Math.sqrt(minDistance2) > unitDistance * 10
        ) {
            return null;
        }
    ...
    const content = prepared ? (
        <EmbeddingView
            ...
            onSelection={(v) => {
                ...
                const newSelectedIndices = v.map(
                    (pt) => pt.identifier as number,
                );
