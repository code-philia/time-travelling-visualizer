    ...
    const togglePlayPause = () => {
        if (isPlaying) {
            ...
            intervalRef.current = setInterval(() => {
                const nextIndex =
                    (currentEpochIndexRef.current + 1) % epochs.length;
                if (nextIndex === 0) {
                    if (intervalRef.current) {
                        clearInterval(intervalRef.current);
                        intervalRef.current = null;
                    ...
                    {nodes.map((node, index) => {
                        if (index < nodes.length - 1) {
                            ...
                            const loadedNodeCount =
                                (progress / 100) * nodes.length;
                            const nextNodeIndex = index + 1;
                            const isLinkLoaded =
                                loadedNodeCount >= nextNodeIndex + 1;
                            return (
                                <line
                                    key={`link-${index}`}
                                    x1={node.x}
...
export function MainBlock() {
    ...
    const { visualizationID, contentPath } = useDefaultStore([
        "visualizationID",
        "contentPath",
    ]);
                ...
                >
                    <Timeline
                        ...
                        onSwitchEpoch={(e) => {
                            ...
                            // Call LazyLoader Function on SwitchEpoch
                            (window as any).fetchHeavyData?.(
                                e,
                                contentPath,
                                visualizationID,
                            );
